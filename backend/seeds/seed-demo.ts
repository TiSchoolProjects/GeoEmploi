...
async function seed() {
  ...
  let seekerUser = await userRepository.findOne({
    where: { email: 'lucas.bernard@mail.fr' },
  });

  if (!seekerUser) {
    seekerUser = await userRepository.save({
      email: 'lucas.bernard@mail.fr',
  ...
  if (!job) {
    job = await jobRepository.save(
      jobRepository.create({
        ...
        title: jobTitle,
        description:
          'NovaTech Solutions recherche un développeur Full Stack pour participer au développement et à la maintenance de ses applications web.',
        commune: 'Paris',
        lat: 48.859,
        lng: 2.347,
  ...
  console.log('Admin      : admin@job-et-bonheur.fr / Admin123!');
  console.log('Employeur  : recrutement@novatech.fr / Employer123!');
  console.log('Candidat   : lucas.bernard@mail.fr / Seeker123!');
  console.log('--------------------------------');
